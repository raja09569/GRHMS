...
class lazy_mofo{
	...
	public $form_text_title_add    = 'Add Guest';   // titles in the <th> of top of the edit form
	...
	//added by Ram
	public $form_text_title_show_full_details   = 'Show Details';

	public $grid_add_link    = "<a href='[script_name]action=edit&amp;[qs]' class='lm_grid_add_link'>Add a Guest</a>";  // link at displayed at the top to add a new record. [script_name] placeholder  will be populated by grid()
	...
	//added by ram
	public $grid_show_full_details_link   = "<a href='[script_name]action=details&amp;[identity_name]=[identity_id]&amp;[qs]'>[details]</a>";
	public $grid_payment_link   = "<a href='[script_name]?action=payment&amp;[identity_name]=[identity_id]&amp;[qs]'>[payment]</a>";
	...
	public $pagination_text_records    = 'Guest(s)';
	...
	function details($error = ''){
		//alert("Iam in full details");
		echo $this->form($error,'details');
	}
	
function payment($error = ''){
		//alert("Iam in full details");
		//echo $this->form($error,'payment');
	...
	function form($error = '',$action){
		...
		/*   if($count == 0){
		 ...
		 $action = 'showfulldetails';
		 $title = $this->form_text_title_show_full_details;
		 // $validate = $this->on_update_validate;
		 }*/
		...
		$html .= "<div class='lm_form_button_bar'>$this->form_back_button </div>";
	...
	function grid($error = ''){
		...
		//added by ram
		$grid_show_full_details_link = $this->grid_show_full_details_link;
		$grid_payment_link = $this->grid_payment_link;
		...
		$grid_show_full_details_link = str_replace('[script_name]', $uri_path, $grid_show_full_details_link);
		$grid_show_full_details_link = str_replace('[qs]', $qs, $grid_show_full_details_link);
		$grid_show_full_details_link = str_replace('[identity_name]', $this->identity_name, $grid_show_full_details_link);

		$grid_payment_link = str_replace('[script_name]', 'guest_payment.php', $grid_payment_link);
		$grid_payment_link = str_replace('[qs]', $qs, $grid_payment_link);
		$grid_payment_link = str_replace('[identity_name]', $this->identity_name, $grid_payment_link);
		
		$links = $grid_edit_link . ' ' . $grid_delete_link.' '.$grid_show_full_details_link.' '.$grid_payment_link;
